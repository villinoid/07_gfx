...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
 ...
 for (point=0; point < polygons->lastcol-1; point+=3){
	if(!((polygons->m[0][point]==polygons->m[0][point+1] &&polygons->m[1][point]==polygons->m[0][point+1] ) || 
	(polygons->m[0][point+1]==polygons->m[0][point+2] && polygons->m[1][point+1]==polygons->m[1][point+2] ) ||
	(polygons->m[0][point+1]==polygons->m[0][point] && polygons->m[1][point+1]==polygons->m[1][point] ))){
		double *N=calculate_normal(polygons,point);
		if(dot_product(N,V)>0){
				...
				draw_line(polygons->m[0][point],polygons->m[1][point],polygons->m[0][point+2],polygons->m[1][point+2],s,c);
		}
	}
 }
}
...
void add_sphere( struct matrix * polygons,
  ...
  int index, index2, index3, lat, longt;
  ...
  for ( lat = latStart; lat <latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
	  ...
	  index2=(index+steps);
	  index3=(index+steps+1);
	  
	  add_polygon(polygons,points->m[0][index], points->m[1][index],points->m[2][index],
...
void add_torus( struct matrix * polygons, 
  ...
  for ( lat = latStart; lat <= latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
	  ...
	  add_polygon(polygons,points->m[0][index], points->m[1][index],points->m[2][index],
						points->m[0][index+1],points->m[1][index+1],points->m[2][index+1],
						points->m[0][index+steps+1], points->m[1][index+steps+1], points->m[2][index+steps+1]);
	  add_polygon(polygons,points->m[0][index], points->m[1][index],points->m[2][index],
						points->m[0][index+steps+1], points->m[1][index+steps+1], points->m[2][index+steps+1],
						points->m[0][index+steps], points->m[1][index+steps], points->m[2][index+steps]);
	}
